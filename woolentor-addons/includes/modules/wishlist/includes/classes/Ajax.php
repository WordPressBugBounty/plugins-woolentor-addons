...
class Ajax {
    ...
    /**
     * [update_quantity] Persist a wishlist item quantity so it survives a page refresh.
     * @return [void]
     */
    public function update_quantity(){
        ...
        $quantity = isset( $_REQUEST['quantity'] ) ? absint( $_REQUEST['quantity'] ) : 0;

        if ( $id > 0 && $quantity > 0 ) {
            $updated = \WishSuite\Frontend\Manage_Wishlist::instance()->update_product_quantity( $id, $quantity );
            if ( false !== $updated ) {
                wp_send_json_success([
                    'message' => __( 'Quantity updated successfully!', 'woolentor' )
                ]);
            }
        }

        wp_send_json_error([
            'message' => __( 'Failed to update quantity!', 'woolentor' )
        ]);
    }
