<?php
/**
 * Product Showcase — Modern / Variant 2.
 *
 * Header split left/right — eyebrow over a large tight headline on the left, a pill filter group
 * on the right — then a four-across grid of borderless portrait cards, and a centred footer that
 * pairs a result count with an outline "View All" button.
 *
 * Reference: design-reference/new_temlate/modern-style/v2/homepage.html:5589 — `.products-grid`
 *
 * The card lives in v2-card.php, which the tab handler re-renders on its own. The outer .wl-ps
...
$wl_ps_view_all = $this->view_all_link( $header['view_all'], $this->arrow( 12 ) );
...
<?php if ( '' !== $header['count_line'] || '' !== $wl_ps_view_all ) : ?>
    <div class="wl-ps-foot-row">
        <?php if ( '' !== $header['count_line'] ) : ?>
            <span class="wl-ps-count-line"><?php echo esc_html( $header['count_line'] ); ?></span>
        <?php endif; ?>
